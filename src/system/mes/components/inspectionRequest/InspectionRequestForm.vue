		...
		<Splitter :paneConfigs="paneConfigs">
			...
			<template v-slot:right>
				<FormGrid
					...
					class="ml-3"
					gridId="Material"
					gridTitle="Inspection Request"
					menuId="InspectionRequest"
				>
...
export default {
	...
	setup() {
		...
		const scanId = ref("InspectionRequest.Scan");
		const scanItems = ref([]);
		...
		function validateInspecteItem() {
			return customItems.value.every(
				item =>
					item.inspectionState === DATA_STATE.WAIT &&
