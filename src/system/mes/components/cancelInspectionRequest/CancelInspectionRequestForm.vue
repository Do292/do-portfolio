		...
		<Splitter :paneConfigs="paneConfigs">
			...
			<template v-slot:right>
				<FormGrid
					...
					class="ml-3"
					gridId="Material"
					gridTitle="Cancel Inspection Request"
					menuId="CancelInspectionRequest"
				>
...
import * as INSPECTION from "../../api/inspection.js";
import * as SYSTEM_COMMON from "~/api/system-common.js";
import InnerScanForm from "../../components/common/InnerScanForm.vue";
import { ACTION, DATA_STATE } from "~/system/mes/constants/mes_constants.js";

export default {
	name: "CancelInspectionRequestForm",
	data() {
		return {
			MATERIAL,
	...
	setup() {
		...
		const scanId = ref("CancelInspectionRequest.Scan");
		const scanItems = ref([]);
		...
		function openModalConfirm() {
			if (!validateCacncelInspecteItem()) {
				openModalWarning(
					"inspectionState, inspectionRequestDate를 확인하세요",
				);
				return;
			}
		...
		async function action(params) {
			let action = ACTION.REQUESTCANCEL;
			const { data } = await INSPECTION.requestItems(params, action);
		...
		function validateCacncelInspecteItem() {
			return customItems.value.every(
				item =>
					item.inspectionState === DATA_STATE.REQUEST &&
					!proxy.$isEmpty(item.inspectionRequestDate),
			);
		}
