			...
			<Splitter :paneConfigs="paneConfigs">
				<template v-slot:left>
					...
					<div class="left-area ml-1">
						<Accordion :canExpand="true" :panelConfig="panelConfig">
...
export default {
	...
	setup(props) {
		...
		function openModalConfirmOrder() {
			...
			var actionType = [props.action];

			items.value.forEach(i => {
				i.eventUser = userId;
				Object.assign(i, panelItem.value);

				// lot ID 일 경우 oldLotId, newLotId를 설정한다.
				if (properties.LOTID == panelItem.value.attributeName) {
					var attr = {
						[properties.OLD_LOTID]: i.lotId,
						[properties.NEW_LOTID]: panelItem.value.attributeValue,
					};

					actionType += "/lotNo";
					Object.assign(i, attr);
				}
			});

			var body = { ...items.value };
			let parameters = { body };
			...
			openModalConfirmBy(
				...
				async () => {
					const result = await executeList(() =>
						// MES_COMMON.doAction(id, actionType, items.value),
						MES_COMMON.doAction(id, actionType, parameters),
					);
