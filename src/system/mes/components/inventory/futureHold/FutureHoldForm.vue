...
export default {
	...
	setup(props) {
		...
		const poGridRef = ref(null);
		const lotGridRef = ref(null);

		const isPanelCollapsed = ref(false);

		// Grid Column
		const poGridColumns = ref([]);
		...
		const lotQuery = ref({
			[CAMEL_ID.QUERY_ID]: "LotInfo",
			[CAMEL_ID.QUERY_VERSION]: "000001",
		});

		const operListQuery = ref({
			[CAMEL_ID.QUERY_ID]: "GetNextOperListByLot",
			[CAMEL_ID.QUERY_VERSION]: "000001",
		});
		...
		function getInformation(item) {
			...
			executeWithSpinner(async () => {
				...
				// Oper List Search
				var searchCondition = { ...operListQuery.value, parameters };
				condition = { searchCondition };

				const operList = await fetchList(() =>
					getByCustomQuery(condition),
				);

				operations.value = [];
				operations.value = [...operList];
		...
		function openModalConfirm() {
			...
			if (lots.value.length == 0) {
				openModalWarning("Lot 정보를 조회 하세요.");
				return;
			}

			var selectedItems = poGridRef.value.cloneCheckItems();

			if (selectedItems.length == 0) {
				openModalWarning("공정 정보를 선택하세요.");
				return;
			}
			...
			lots.value.forEach(i => {
				i.eventUser = userId;
				i.processFlowId = selectedItems[0].processFlowId;
				i.processFlowId = selectedItems[0].processOperationId;
