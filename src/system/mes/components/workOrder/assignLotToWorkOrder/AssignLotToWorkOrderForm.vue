...
export default {
	...
	setup(props) {
		...
		onBeforeMount(async () => {
			// // Product
			// await fetchGridColumns(props.menuId, props.woGridId);

			// // gridColumn -> woGridColumns 로 데이터를 옮긴다.
			// woGridColumns.value = JSON.parse(JSON.stringify(gridColumns.value));

			// // Lot
			// await fetchGridColumns(props.menuId, props.gridId);
			await fetchMetaColumns(props.metaDataId);

			queryInfos[WORK_ORDER.ID] = initQueryConfig(
				props.menuId,
				props.woGridId,
			);

			queryInfos[LOT.ID] = initQueryConfig(props.menuId, props.gridId);
		});
		...
		function getInformation(item) {
			...
			if (LOT.ID == item.field) {
				...
				isExist.value = lots?.value.find(
					lot => lot[WORK_ORDER.ID] == panelItem.value[item.field],
				);

				isLot.value = true;
			} else if (WORK_ORDER.ID == item.field) {
				// Workorder Field 확인
				isExist.value = workorders?.value.find(
					workorder =>
						workorder[LOT.ID] == panelItem.value[item.field],
				);
			}
			...
			executeWithSpinner(async () => {
				...
				if (isLot.value) {
					...
					workorders.value = [];
					workorders.value = [...workorders.value, queryItem];
				}
