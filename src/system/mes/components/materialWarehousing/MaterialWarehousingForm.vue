		...
		<Splitter :paneConfigs="paneConfigs">
			...
			<template v-slot:right>
				<FormGrid
					...
					class="ml-3"
					gridId="Material"
					gridTitle="Material Warehousing"
					menuId="MaterialWarehousing"
				>
...
export default {
	...
	setup() {
		...
		const scanId = ref("MaterialWarehousing.Scan");
		const scanItems = ref([]);
		...
		function validateApplyItems() {
			return customItems.value.every(
				item => item.materialState === DATA_STATE.CREATED,
			);
		}
