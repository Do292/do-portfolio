...
export default {
	...
	setup(props, { emit }) {
		...
		const gridColumns = computed(() => {
			const isFreezing = column =>
				column[ID.FIXING_FLAG] === FLAG.Y ||
				(props.freezing && checkColumnPrimary(column));
		...
		const gridTargetRef = ref(null);

		/**
		 * @param {KeyboardEvent} event
		 * @param {HTMLElement} target
		 */
		const textValidatorInGrid = ({ target }) =>
			target.classList.contains(FOCUS_CELL) ? target.textContent : null;

		onMounted(() => {
			// Grid Content로 타겟 지정
			gridTargetRef.value =
				gridRef.value.$el.querySelector(".e-gridcontent");

			// 버튼 이벤트
			const buttonConfigs = [defineButtonConfig(KEY.EXPORT, exportExcel)];

			// Import여부 따라 이벤트 부여
			if (props.importing) {
				buttonConfigs.push(
					defineButtonConfig(KEY.IMPORT, () =>
						importRef.value.click(),
					),
				);
			}
			activateButtonEvent(gridTargetRef, buttonConfigs);

			// 카피 이벤트
			activateCopyEvent(gridTargetRef, textValidatorInGrid);
		});
		...
		const editSetting = { allowDeleting: props.deleting, mode: "Normal" };
		...
		const dropTarget = ref(props.dropTargetId ?? props.gridName);
		...
		function importExcel({ target }) {
			...
			reader.onload = ({ target: { result } }) => {
				...
				const rowItems = Object.values(Sheets).flatMap(sheet => {
					...
					// 대문자 헤더를 프로퍼티로 하는 객체 생성
					const keys = headers.map(header => header.toUpperCase());
...
/* Paging */
:deep(.e-grid .e-pager .e-pagerconstant) {
	display: none;
}

/* FrozenColumn 관리 */
:deep(.e-grid .e-gridcontent .e-content) {
	height: 100% !important;
}
:deep(.e-grid .e-movablescrollbar) {
	transform: translateY(-8px);
	overflow-x: auto !important;
	max-height: 8px !important;
}
:deep(.e-grid .e-movablecontent) {
	overflow-x: auto;
	overflow-y: hidden;
...
/* Drag and Drop */
:deep(.e-draganddrop.e-grid) {
	max-height: 30px !important;
}
:deep(table.dragging) {
	border-bottom: 2.5px #634df5 solid !important;
}
:deep(.dragging-row) {
	background: #e6e6fa;
}
:deep(.cross-draggable .e-selectionbackground) {
	cursor: move;
}
...
/* Filter */
:deep(th.filtered .e-headertext) {
	opacity: 1;
	color: black;
}
:deep(td > span.filtered) {
	background: #ffe5d977;
	color: black;
	font-weight: 500;
}
:deep(.e-grid .e-gridheader .e-sortfilter .e-fltr-icon .e-filtermenudiv) {
	margin-right: 0px !important;
}

/* Search Filter */
:deep(.e-filter-popup .e-dlg-content) {
	padding-right: 13px !important;
}
:deep(.e-searchbox.e-fields .e-input-group) {
	width: 100% !important;
}
...
:deep(.type span.String),
:deep(.type span.Label),
:deep(.type span.Create),
:deep(.type span.Engineer) {
	background: #edf8ea;
	color: #18bf18;
}
:deep(.type span.Never),
:deep(.type span.Rework),
:deep(.type span.BUMP) {
	background: #fce4ec;
	color: #d81b60;
}
:deep(.type span.Long),
:deep(.type span.Always),
:deep(.type span.Logical),
:deep(.type span.Message),
:deep(.type span.Production),
:deep(.type span.Operator),
:deep(.type span.Process) {
	background: #e3f2fd;
	color: #1e88e5;
}
:deep(.type span.Main),
...
:deep(.state span.Created) {
	background: #e6f9e6;
	color: #05c645;
}
:deep(.state span.NotActive) {
	background: #fce4ec;
	color: #d81b60;
}
:deep(.state span.CheckedIn),
:deep(.state span.Active) {
	background: #e2eafd;
	color: #0744cc;
}
