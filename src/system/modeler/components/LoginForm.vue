...
import ComboLocale from "~/components/common/combo/ComboLocale";
...
import {
	...
	useEncryt,
	useMenu,
} from "~/plugins/composables/authority";

import * as USER_SETTING from "~/api/user.js";
import { USER } from "~/constants/common_constants.js";

export default {
	name: "LoginForm",
	components: { InputUserId, InputPassword, ComboLocale },
	setup() {
		//==================== Config
		...
		async function logIn() {
			const data = await fetchOne(() =>
				USER_SETTING.logIn(userId.value, encryptValue(password.value)),
			);
			...
			if (data) {
				...
				// 언어 설정
				i18n.locale.value = language.value ?? data[USER.LANGUAGE];
